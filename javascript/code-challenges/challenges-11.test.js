...
const count = (target, input) => {
  // return input.map(array => array.filter(value => value === target)).map(value => value.filter(value => value.length !== 0)).length;
  let sum = 0;
  input.map(value => value.map(value => value === target ? sum += 1 : sum));
  return sum;
};
...
const totalSum = (input) => {
  return input.flat().reduce((pv, cv) => pv + cv);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(array => array.filter(num => typeof(num) === 'number')).map(array => array.filter(num => num % 5 ===0)).map(array => array.map(number => Math.pow(2, number)));
};
...
let findMaleAndFemale = (data) => {
  return data.filter(object => object.gender === 'male' || object.gender === 'female').map(character => character.name).join(' and ');
};
