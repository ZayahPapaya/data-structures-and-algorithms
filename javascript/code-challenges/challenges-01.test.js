...
const addOne = (arr) => {
  let arr2 = [];
  arr.forEach(value => arr2.push(value + 1));
  return arr2;
};
...
const speaker = (words, callback) => {
  let arr = [];
  words.forEach(value => arr.push(callback(value)));
  return arr;
};
...
const createList = (availableItems) => {
  let arr2 = [];
  availableItems.forEach(value => {if(value.available === true){arr2.push(value.name);}});
  return arr2;
};
