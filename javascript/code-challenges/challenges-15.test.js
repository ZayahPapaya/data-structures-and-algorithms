...
const screenForNames = (arr) => {
  const a = /^[MD][rs]+.\s[A-Za-z]+/gm;
  return arr.filter(val => a.test(val));
};
...
let biggerThanLuke = (arr) => {
  // const dumpy = arr.find(character => character.name === 'Luke Skywalker');
  // return arr.filter(character => parseInt(character.mass) > parseInt(dumpy.mass)).reduce((acc, currVal, idx, arr) => {
  //   let delimiter = '';
  //   if (idx === arr.length - 1) {
  //     delimiter = ' - ';
  //   }
  ...
  // return arr.filter(character => parseInt(character.mass) > parseInt(arr.find(character => character.name === 'Luke Skywalker').mass)).reduce((acc, currVal, idx, arr) => {
  //   let delimiter = '';
  //   if (idx === arr.length - 1) {
  //     delimiter = ' - ';
  //   }
  ...
  //   return acc;
  // }, '');
  return arr.filter(val => val.mass > 77).map(val => val.name).join(' - ');
};
