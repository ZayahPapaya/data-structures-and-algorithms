...
const longestString = (arr) => {
  let pog = '';
  arr.forEach(string => {
    if (string.length > pog.length) {
      pog = string;
    }
  });
  return arr.findIndex(string => string === pog);
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(phone => `${phone.substring(1, 4)}${phone.substring(6, 9)}${phone.substring(10, 15)}`);
};
...
const allHappy = (arr) => {
  return arr.filter(value => value.includes(':)')).length === arr.length ? true : false;
};
